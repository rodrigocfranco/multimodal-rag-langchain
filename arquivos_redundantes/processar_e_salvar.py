...
# 🔥 CORREÇÃO: Extrair com chunking para agregar elementos relacionados
chunks = partition_pdf(
    filename=file_path,
    infer_table_structure=True,            # Detectar tabelas
    strategy="hi_res",                     # Alta resolução
    extract_image_block_types=["Image", "Table"],  # Extrair imagens e tabelas
    extract_image_block_to_payload=True,   # Base64 para API
    chunking_strategy="by_title",          # Agrupa por título (ESSENCIAL!)
    max_characters=10000,                  # Tamanho máximo do chunk
    combine_text_under_n_chars=2000,       # Combina textos pequenos
    new_after_n_chars=6000,                # Quebra após N caracteres
)
